...
| 상품 좋아요 토글 | PATCH | `/v1/products/{productId}/like` |
...
- 응답 배열 순서는 `display_order ASC, id ASC` 기준이며 클라이언트는 응답 순서대로 렌더링한다.
- 캐러셀이 없으면 `carousels: []`를 반환한다.
- `imageUrl`은 S3 presigned URL 형식으로 반환한다.
...
```json
{
  ...
  "pageInfo": {
    "nextCursor": "eyJzZWN0aW9uRGlzcGxheU9yZGVyIjoxLCJzZWN0aW9uSWQiOjEsInNob3djYXNlRGlzcGxheU9yZGVyIjoxMSwic2hvd2Nhc2VJZCI6MTF9",
    "hasNext": true,
    "size": 12
  }
}
...
| `sections[].showcases` | Array | 현재 페이지에 해당하는 섹션 하위 쇼케이스 목록 |
...
- `featured`와 일반 `showcases`에 같은 쇼케이스가 중복 포함될 수 있다.
- `displayOrder`는 서버 내부 정렬 및 seed 관리용 필드이며 API 응답에는 포함하지 않는다.
- cursor는 서버 내부적으로 `sectionDisplayOrder`, `sectionId`, `showcaseDisplayOrder`, `showcaseId` 기준 값을 Base64 인코딩해 생성한다.
- `pageInfo.hasNext=false`이면 더 이상 요청하지 않는다.
- pagination 단위는 showcase 개수 기준이므로, 동일한 `sectionId`가 여러 페이지에 걸쳐 반복될 수 있다. 클라이언트는 `sections`를 단순 append하지 않고 `sectionId` 기준으로 showcases를 병합해야 한다.
- `Event`, `Lookbook` 탭처럼 쇼케이스 내부 필터가 실제 동작한다면 `type` query param 추가를 검토한다.

## 10. 상품 좋아요 토글

### `PATCH /v1/products/{productId}/like`
...
아래 API는 S3 연동 확인을 위한 테스트용 API이며, 클라이언트 정식 API 계약 범위에는 포함하지 않는다.

| 기능 | Method | Path |
| --- | --- | --- |
| S3 버킷 연결 확인 | GET | `/v1/test/s3/connection` |
| S3 객체 목록 조회 | GET | `/v1/test/s3/list` |
| S3 Presigned URL 생성 | GET | `/v1/test/s3/presigned-url` |
