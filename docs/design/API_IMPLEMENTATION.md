...
- ShowCase cursor payload에는 `displayOrder`, `sectionId`만 포함한다.
...
- 일반 쇼케이스 pagination: `showcases.display_order ASC, id ASC`
...
- 응답 DTO 조립 시 이미지 객체 키는 `S3Service.generatePresignedUrl()`로 변환한다.
