...
- ShowCase cursor payload는 `displayOrder`, `sectionId`를 포함한다.
...
5. 일반 feed는 featured 여부와 무관하게 전체 feed 대상으로 pagination한다.
6. theme이 있으면 해당 theme의 feed만 조회한다.
7. feed를 section 단위로 묶는다.
8. `pageInfo`를 생성한다.
9. 응답 DTO를 조립한다.
...
- ShowCase cursor payload에는 `displayOrder`, `showcaseId`만 포함한다.
...
- `is_featured=true` 데이터가 없으면 전체 feed 중 `display_order` 기준 마지막 2개를 사용한다.
- fallback featured는 `display_order DESC, id DESC LIMIT 2`로 최신 2개를 선택한 뒤, 응답 조립 시 `display_order ASC, id ASC`로 재정렬해 기존 feed 순서를 유지한다.
- cursor가 있는 후속 요청에서는 `featured=[]`
- featured와 일반 feed 중복은 허용한다.

정렬:

- 일반 feed pagination: `showcases.display_order ASC, id ASC`
- section 응답: `showcase_sections.display_order ASC, id ASC`
