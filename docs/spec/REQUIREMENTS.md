...
- 더미데이터 seed에는 최종 이미지 URL을 사용한다.
- 기획 전달 JSON에는 이미지 식별자 또는 파일명을 포함할 수 있다.
...
- 응답 배열은 노출 순서대로 정렬한다.
- 클릭 이동이 없는 캐러셀은 `linkUrl=null`로 응답할 수 있다.
...
| `carousels[].linkUrl` | String or null | 클릭 시 이동할 URL |
...
- Product Like 기반 정렬은 Main 상품 영역에만 적용한다.
- ShowCase 데이터는 포함하지 않는다.
- Product Like 기반 정렬은 좋아요 수를 우선 기준으로 사용하고, 동률이면 기존 노출 순서를 유지한다.
...
- 날짜는 원본 날짜와 화면 표시용 문구를 함께 응답한다.
- 날짜는 현재 표시용으로 사용하되, 추후 필터 기준으로 확장할 수 있도록 보관한다.
- featured와 일반 feed는 같은 ShowCase 테이블에서 관리한다.
- 기획 전달 JSON에서 `isFeatured`로 featured를 명시하면 해당 피드를 featured로 사용한다.
- `isFeatured`가 없으면 전체 ShowCase feed의 index 기준 마지막 2개를 featured로 선정한다.
- featured 응답 순서는 기존 feed 순서를 유지한다.
- featured 항목이 일반 feed에 중복 포함되는 것은 허용한다.
- featured는 `cursor`가 없는 첫 요청에서만 응답한다.
- `cursor`가 있는 후속 요청에서는 `featured=[]`로 응답한다.
- 일반 `feeds`는 featured 포함 여부와 무관하게 전체 ShowCase feed를 대상으로 pagination한다.
...
| `sections[].feeds` | Array | 섹션 하위 쇼케이스 카드 목록 |
| `sections[].feeds[].showcaseId` | Long | 쇼케이스 ID |
| `sections[].feeds[].title` | String | 쇼케이스 제목 |
| `sections[].feeds[].description` | String | 쇼케이스 설명 |
| `sections[].feeds[].imageUrl` | String | 쇼케이스 이미지 URL |
| `sections[].feeds[].startDate` | LocalDate | 쇼케이스 시작일 |
| `sections[].feeds[].endDate` | LocalDate | 쇼케이스 종료일 |
| `sections[].feeds[].displayPeriod` | String | 화면 표시용 기간 문구 |
| `pageInfo.nextCursor` | String or null | 다음 요청에 사용할 cursor |
| `pageInfo.hasNext` | Boolean | 다음 페이지 존재 여부 |
| `pageInfo.size` | Integer | 요청 또는 적용된 페이지 크기 |
...
- 노출 가능한 섹션이 없으면 `sections=[]`, `pageInfo.hasNext=false`, `pageInfo.nextCursor=null`을 반환한다.
- 특정 섹션에 피드가 없으면 해당 섹션의 `feeds=[]`를 반환할 수 있다.
...
| 항목 | 현재 가정 | 확인 필요 내용 |
| --- | --- | --- |
| 이미지 전달 및 업로드 | seed에는 최종 이미지 URL 사용 | S3 업로드 담당자와 bucket/path 규칙 확인 |
